...
HugeInt HugeInt::operator-( const HugeInt &desperate ) const
{
    HugeInt temp;// temporary result
    int borrow=0;
    for ( int i = 29; i >= 0; i-- )
   {
       temp.integer[i]=integer[i]-desperate.integer[i]-borrow;
       // determine whether to borrow a 1
       if(temp.integer[i]<0)
       {
           temp.integer[i]+=10;
           borrow=1;
...
HugeInt HugeInt::operator-( int desperate) const
{
    return *this - HugeInt(desperate);// convert desperate to a HugeInt, then invoke
   // operator- for two HugeInt objects
}
HugeInt HugeInt::operator-( const char *desperate )const
{
    return *this - HugeInt(desperate);// convert desperate to a HugeInt, then invoke
   // operator- for two HugeInt objects
}
...
bool HugeInt::operator==(HugeInt &t)
{
    for(int i=0;i<30;i++)
    {
        if(integer[i]!=t.integer[i])
            return false;
    }
    return true;
}
